app.controller('AdminDoctorCalendarController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route, API, adminBreadcrumbService,convertMoneyService) {
    ...
    $scope.getCTResultAbnormalityByAppointment = (appointmentId) => {
        $http.get(url + "/ct-result-abnormality-by-appointmnet-id/" + appointmentId, { headers: headers }).then((response) => {
            $scope.listCTResultAbnormalityByAppointment = response.data
        })
    }
    ...
    $scope.initializeUIComponentsModal = () => {
        ...
        $('#treatmentDurationId').on('change', function () {
            $timeout(function () {
                ...
                console.log("selectedVal kkkk", $scope.selectedTreatmentDuration);

                $scope.selectedTreatmentDuration = processSelect2Service.processSelect2Data(selectedVal)
                console.log("selectedVal huhu", $scope.selectedTreatmentDuration);
