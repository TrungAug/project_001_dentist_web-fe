    ...
    $scope.getDoctorScheduleByTimeRange = (startStr, endStr, paramsFiler) => {
        ...
        if (!paramsFiler) {
            ...
            $http.get(url + '/map-ds-by-time-range', { params: params, headers: headers }).then(response => {
                ...
                dataResponse.forEach(datas => {
                    datas.forEach(item => {
                        if (item.doctor) {
                            // Kiểm tra xem bác sĩ đã tồn tại trong danh sách chưa
                            const exists = $scope.listDoctorScheduleByTimeRangesDB.some(doc => doc.id === item.doctor.id);
                            if (!exists) {
                                $scope.listDoctorScheduleByTimeRangesDB.push(item.doctor);
                            }
                        }
                    });
                });
