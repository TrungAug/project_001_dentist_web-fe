app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.calculateGrowthPercentage = (todayLength,previousLength) => {
     todayLength = parseFloat(todayLength);
     previousLength = parseFloat(previousLength);
    let appFluctuate=0
     if (isNaN(todayLength) || isNaN(previousLength)) {
         return appFluctuate
     }
     ...
     if (previousLength === 0) {
         ...
         let growth = ((todayLength - previousLength) / previousLength) * 100;
