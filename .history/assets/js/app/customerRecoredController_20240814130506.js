    ...
    $scope.cancelAppoinmnet=(appointment)=>{
        ...
        Promise.all([duRequest, appServiceRequest]).then(([duResponse,appServiceResponse])=>{
            ...
            duResponse.data.forEach(du=>{
                ...
                duPutRequest.push($http.put(url+`/appointment-patient-record/${id}`, angular.toJson(appointmentPatientRecordRequest), { headers: headers }))
            })
            console.log("duPutRequest",duPutRequest);
