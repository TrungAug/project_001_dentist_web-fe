app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchTotalRevenueByYear = (dateString) => {
    ...
    const prevDate = processParam.day ? moment(processParam.day, 'DD/MM/YYYY').subtract(1, 'days').format('YYYY-MM-DD') : null
    const prevParam = {
      date: prevDate,
  ...
  $scope.fetchRevenueByDateOrMonth = (dateString) => {
    ...
    const param = {
      date: processParam.day ? moment(processParam.day, 'DD/MM/YYYY').format('YYYY-MM-DD') : null,
  ...
  //     params.day = date.getDate().toString().padStart(2, '0');
  //     params.month = (date.getMonth() + 1).toString().padStart(2, '0');
  //     params.year = date.getFullYear();
  //   } catch (error) {
  //     console.error('Invalid date format:', error);
  //     throw new Error('Không đúng định dạng ngày. Vui lòng cung cấp định dạng YYYY-MM-DD.');
  //   }
