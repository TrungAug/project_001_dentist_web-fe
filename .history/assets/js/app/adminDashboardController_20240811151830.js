app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchTotalRevenueByYear = (dateString) => {
    ...
    let processParam = $scope.generateParamsForGetData("11/08/2024")
    console.log("processParam huu",processParam);
    
    const currentYear = parseInt(processParam.year);
    const prevYear = currentYear - 1
    const param = $scope.generateParamsForGetData(currentYear.toString())
    const prevDate = processParam.date ? moment(processParam.date, 'DD/MM/YYYY').subtract(1, 'days').format('YYYY-MM-DD') : null
    const prevParam = {
      date: prevDate,
    ...
    console.log("prevParam xx",prevParam);
  ...
  $scope.fetchRevenueByDateOrMonth = (dateString) => {
    ...
    const param = {
      date: processParam.date ? moment(processParam.date, 'DD/MM/YYYY').format('YYYY-MM-DD') : null,
  ...
  $scope.fetchListAppointmentByYear = async (dateString) => {
    const params = await $scope.generateParamsForGetData("11/08/2024");
