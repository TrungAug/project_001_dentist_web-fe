    ...
    $scope.getDoctorScheduleByTimeRange = (startStr, endStr, paramsFiler) => {
        ...
        if (!paramsFiler) {
            ...
            //    $scope.listDoctorScheduleByTimeRangesDB = response.data
                ...
                $scope.listDoctorScheduleByTimeRangesDB = response.data
                    .flatMap(datas => datas.filter(item => item.doctor))  // Lọc các phần tử có bác sĩ
                    .reduce((acc, item) => {  // Loại bỏ trùng lặp
                        if (!acc.some(doc => doc.id === item.doctor.id)) {  // Giả sử mỗi bác sĩ có thuộc tính id
                            acc.push(item.doctor);
                        }
                        return acc;
                    }, []);
