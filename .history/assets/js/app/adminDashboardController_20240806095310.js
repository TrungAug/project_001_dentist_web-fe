app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.resetMonth=($event)=>{
    $event.preventDefault()
    $scope.formFilter.month = moment(new Date()).format('MM-YYYY')
    $('#filterByMoth').val($scope.formFilter.month)
    $scope.filterByMoth = $scope.formFilter.month
    $scope.chartArea($scope.filterByMoth)
  }
  ...
  $scope.initUI = () => {
    ...
    $('#filterByMoth').on('change', function () {
      $timeout(function () {
        let selectedVals = $('#filterByMoth').val()
        $scope.filterByMoth = selectedVals.split(':')[1]
        $scope.chartArea($scope.filterByMoth)
      });
    });
  }
  ...
  $scope.initUI()
  $scope.getListAppointment()
  $scope.chartArea($scope.formFilter.month)
  $scope.generateMMYYYYData()
})
