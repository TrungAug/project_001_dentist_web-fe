app.controller('LoginController', function ($scope, $http, $rootScope, $location, $timeout, $window, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.getParameterByName = (name, url = window.location.href) => {
        name = name.replace(/[\[\]]/g, '\\$&');
    ...
    let code = $scope.getParameterByName('code');
    let loginSource = $scope.getCookie ('loginSource');
    console.log(loginSource)
    if (code) {
        // Gửi mã code đến backend để lấy access_token và thông tin người dùng
        fetch('http://localhost:8081/api/v1/auth/google/callback', {
            method: 'POST',
            headers: {
