app.controller('AdminDoctorCalendarController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.initializeUIComponents=()=>{
        ...
        $('.drgpicker-doctor-calendar-single-filter').daterangepicker(
            {
        ...
        $('.drgpicker-doctor-calendar-single-filter').on('apply.daterangepicker', function (ev, picker) {
            let selectedDate = $('#drgpicker-doctor-calendar-single-filter').val();
            let paramsFiler = {
    ...
    $scope.getDoctorScheduleByTimeRange = (startStr, endStr, paramsFiler) => {
        ...
        if (!paramsFiler) {
            ...
            $http.get(url + '/map-date-ds-by-time-range', { params: params }).then(response => {
                ...
                console.log("$scope.listDoctorByTimeRange",$scope.listDoctorByTimeRange);
            })
        }
    }
