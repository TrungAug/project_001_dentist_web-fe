app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchListAppointmentByDay = (dateString) => {
    $scope.fetchAppointments(dateString, 'listAppointmentDBToday');
  };

  $scope.fetchListAppointmentByPreviousDay = (dateString) => {
    const prevDateString = moment(dateString).subtract(1, 'days').format('DD/MM/YYYY');
    $scope.fetchAppointments(prevDateString, 'listAppointmentDBPreviousDay');
  };
