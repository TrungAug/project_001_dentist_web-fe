app.controller('LoginController', function ($scope, $http, $rootScope, $location, $timeout, $window, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.login = () => {
        ...
        if (checkForm) {
            ...
            $http.post(url + '/authenticate', AuthenticationRequest).then(response => {
                console.log("$scope.formLogin", response.data);
                localStorage.setItem('accessToken', response.data.access_token);
                localStorage.setItem('refreshToken', response.data.refresh_token);


                // let userString=response.data.user.id + '-' + response.data.user.role.roleName + '-' + response.data.user.email;
                // localStorage.setItem('userLogin', userString);
                let userString = ""
                if (response.data.user.role.roleName == 'BENH_NHAN') {
                    userString = response.data.patient.patientId + '-' + response.data.user.role.roleName + '-' + response.data.user.email;
                } else if (response.data.user.role.roleName == 'BAC_SI') {
                    userString = response.data.doctor.doctorId + '-' + response.data.user.role.roleName + '-' + response.data.user.email;
                } else {
                    userString = response.data.dentalStaff.dentalStaffId + '-' + response.data.user.role.roleName + '-' + response.data.user.email;
                }
                localStorage.setItem('userLogin', userString);
    ...
    $scope.loginWithFacebook = async function () {
        console.log('Logging in with Facebook');
        const cookieExists = await checkCookie("loginSource");
        if (cookieExists) {
            await deleteCookie("loginSource");
        }
        await setCookie("loginSource", "facebook", 1);
        console.log('Cookie after setting:', document.cookie);
        let facebookAuthUrl = 'https://www.facebook.com/dialog/oauth?client_id=894672849328719&redirect_uri=http://localhost:5501/index.html';
