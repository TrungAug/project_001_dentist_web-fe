...
app.controller('AdminPatientRecord', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getAppointmentClass = function (app) {
        console.log("app", app);
        let status = app.appointmentStatus.status.toLowerCase()
        switch (status) {
            case 'đã xác nhận':
                $scope.addClass = 'item-success';
            case 'hoàn thành':
                $scope.addClass = 'item-primary';
            case 'đang diễn ra':
                $scope.addClass = 'item-info';
            case 'đã hủy':
                $scope.addClass = 'item-danger';
            case 'không đến':
                $scope.addClass = 'item-warning';
            default:
                $scope.addClass = '';
        }
    };
