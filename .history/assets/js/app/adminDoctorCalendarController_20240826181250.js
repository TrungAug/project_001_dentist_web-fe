app.controller('AdminDoctorCalendarController', function ($scope, SocketService, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route, API, adminBreadcrumbService, convertMoneyService) {
    ...
    $('#doctorUpdateAppointment').on('shown.bs.modal', function () {
        ...
        $timeout(() => {
            document.getElementById('btn-js-active').addEventListener('click', function (event) {
                // Kiểm tra xem nút có bị disabled hay không
                if (this.disabled) {
                    // Hiển thị thông báo nếu nút bị disabled
                    alert('Nút này đang bị vô hiệu hóa và không thể nhấn.');
                    // Ngăn chặn hành vi mặc định của nút (nếu cần thiết)
                    event.preventDefault();
                } else {
                    // Thực hiện hành động khi nút không bị disabled
