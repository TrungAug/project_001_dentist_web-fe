app.controller('AdminDoctorCalendarController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getAllAppointmentGroupByDate=(paramsFiler)=>{
        ...
        $http.get(url + '/appointment-group-by-date', { params: params }).then(response => {
            ...
            console.log("$scope.listAppointmentByDateAndDoctor",$scope.listAppointmentByDateAndDoctor)
        })
    }
    ...
    $scope.getAppointmentByDateAndDoctor=(list)=>{
        ...
        appCompleted = arr.filter(app => app.appointmentStatus.status.toLowerCase() === 'đã hoàn thành')
        console.log("appCompleted",appCompleted);
        return arr
    }
    ...
    $scope.getDoctorScheduleByTimeRange = (startStr, endStr, paramsFiler) => {
        ...
        if (!paramsFiler) {
            ...
            $http.get(url + '/map-date-ds-by-time-range', { params: params }).then(response => {
                ...
                $scope.isShowDetailDSSingle = Object.keys(response.data).length != 0
