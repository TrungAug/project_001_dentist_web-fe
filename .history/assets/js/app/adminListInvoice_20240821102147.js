app.controller('AdminListInvoice', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route, API, adminBreadcrumbService) {
    ...
    //Them code moi
    // $scope.completedStatusId=null
    $scope.getAppointmentWithOutBill = async () => {
        // $http.get(url+"/appointment-status-except-deleted",{headers:headers}).then(response=>{
        //     let completedStatus=response.data.filter(st=>st.status.toLowerCase()=='hoàn thành'); 
        //     $scope.completedStatusId=completedStatus[0].appointment_StatusId        
        // })
        let responseStatus = await $http.get(url + "/appointment-status-except-deleted", { headers: headers });
        ...
        if (completedStatus.length > 0) {
            ...
            let param={
                appStatus:completedStatusId
            }
            let responseAppointment = await $http.get(url + "/appointment-without-bill", {params:param ,headers: headers })
            $scope.listAppointmentWithOutBill=responseAppointment.data
            console.log("$scope.listAppointmentWithOutBill",$scope.listAppointmentWithOutBill);
