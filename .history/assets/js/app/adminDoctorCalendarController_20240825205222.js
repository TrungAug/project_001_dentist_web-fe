app.controller('AdminDoctorCalendarController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route, API, adminBreadcrumbService, convertMoneyService) {
    ...
    $scope.getCTResultAbnormalityByAppointment = (appointmentId) => {
        ...
        response.data.forEach(rs => {
            ...
            // Assuming appointmentCTResult has an 'id' property you want to use
            let appointmentId = appointmentCTResult.id || JSON.stringify(appointmentCTResult);

            if (!resultMap[appointmentId]) {
                resultMap[appointmentId] = [];
            }

            resultMap[appointmentId].push(abnormality);
        });

        // Transform the resultMap into the desired array format
        $scope.listCTResultAbnormalityByAppointment = Object.keys(resultMap).map(appointmentId => {
            // Convert the id back to the appointmentCTResult object if needed
            let appointmentCTResult = response.data.find(rs => rs.appointmentCTResult.id === appointmentId).appointmentCTResult;
            return [appointmentCTResult, resultMap[appointmentId]];
        });
            console.log("$scope.listCTResultAbnormalityByAppointment", $scope.listCTResultAbnormalityByAppointment);
