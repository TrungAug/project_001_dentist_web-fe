app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.initData = () => {
    ...
    // $scope.growthPercentageMonth = $scope.getRevenueGrowthComparison("", moment().subtract(1, 'months').month().toString(), "")
    // $scope.growthPercentageYear = $scope.getRevenueGrowthComparison("","", moment().subtract(1, 'years').year().toString())
  ...
  $scope.fetchTotalRevenueByPreviousYear = (prevParam) => {
    console.log("prevParam kuku",prevParam);
  ...
  $scope.getRevenueGrowthComparison = (date, month, year) => {
    ...
    if (month && $scope.venueByMonth !== undefined && $scope.revenuePreviousMonth !== undefined && year) {
      let isDate = false
      let isMonth = true
      let isYear = false
      console.log("$scope.venueByMonth",$scope.venueByMonth);
      
      updateGrowthData($scope.venueByMonth, $scope.revenuePreviousMonth, isDate, isMonth, isYear);
    }

    if (year && $scope.venueTotal !== undefined && $scope.venuePrviousYear !== undefined && !month) {
      let isDate = false
      let isMonth = false
      let isYear = true
      console.log("$scope.venueTotal",$scope.venueTotal);
      console.log("$scope.venuePrviousYear",$scope.venuePrviousYear);
