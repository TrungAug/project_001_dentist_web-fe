    ...
    $scope.getDoctorScheduleByTimeRange = (startStr, endStr, paramsFiler) => {
        ...
        if (!paramsFiler) {
            ...
            $http.get(url + '/map-ds-by-time-range', { params: params, headers: headers }).then(response => {
                ...
                //             .filter(ds => ds.doctor) 
                //             .filter(ds => {
                //                 if (!seenDoctorIds.has(ds.doctor.doctorId)) { 
                //                     seenDoctorIds.add(ds.doctor.doctorId);
                //                     return true;
                //                 }
                //                 return false;
                //             });
                ...
                Object.values(response.data).forEach(schedules => {
                    schedules.forEach(schedule => {
                        uniqueDoctors.add(schedule.doctor.doctorId);
                    });
                });

               // $scope.listDoctorScheduleByTimeRangesDB=[...uniqueDoctors]
                console.log("aa",[...uniqueDoctors]);
