...
app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService) {
  ...
  $scope.fetchRevenueByPastDate = (date) => {
    ...
    $scope.getRevenue(pastDate).then(response => {
      ...
      if ($scope.venuePastToday === 0) {
        $scope.venuePastToday = 1;
      }
  ...
  $scope.getRevenueGrowthComparison = () => {
    console.log("$scope.growthPercentage",$scope.growthPercentage);
    console.log("$scope.venueToday",$scope.venueToday);
    console.log("$scope.venuePastToday",$scope.venuePastToday);
    ...
    if ($scope.venueToday !== undefined && $scope.venuePastToday !== undefined) {
      ...
      if (pastRevenue > 0) {
        let growthPercentage = ((todayRevenue - pastRevenue) / pastRevenue) * 100;
        $scope.growthPercentage = growthPercentage.toFixed(2) + '%';
      } else {
        $scope.growthPercentage = 'N/A'; // Trường hợp không có doanh thu ngày hôm qua
      }
  ...
  $scope.fetchRevenueByPastDate($scope.today)
  $scope.fetchRevenueByDate($scope.today)
})
