app.controller('AdminPatientController', function ($scope, $http, $rootScope, $location, $timeout, API, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.showHistorySelected = (id) => {
        ...
        let check =$scope.listMedicalHistoryDB.filter(item => item.medicalHistoryId == id)
        console.log("chekc",check.name);
        
        return $scope.listMedicalHistoryDB.filter(item => item.medicalHistoryId == id).name
    }
    ...
    $scope.getMedicalHistories = function () {
        $http.get(url + '/medical-history', { headers: headers }).then(function (response) {
            ...
            console.log("$scope.listMedicalHistoryDB", $scope.listMedicalHistoryDB);
