app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.resetMonth = () => {
    ...
    $scope.formFilter.d = $scope.dateDataDB.find(item => item === $scope.filterByMonth)
    console.log("$scope.formFilter.d", $scope.formFilter.d);
  }
  ...
  $scope.initUI = () => {
    ...
    $('#filterByMonth').on('change', function () {
      $timeout(function () {
        ...
        $scope.filterByMonth = selectedVals.split(':')[1]
