    ...
    $scope.crudDoctor = () => {
        ...
        async function updateImageToCloudinary(files, publicId) {
            ...
            try {
                ...
                console.error('Error updating image to Cloudinary:', error);
                throw error; 
            }
        }
        ...
        $scope.updateDoctor = async () => {
            console.log("$scope.fileDoctorCloudinary", $scope.fileDoctorCloudinary);
            ...
            if (valid) {
                ...
                try {
                    ...
                    const doctorId = response.data.doctorId;
                    // await Promise.all([                      
                    //     updateImageToCloudinary($scope.fileDoctorCloudinary, `doctor_${doctorId}_profile_picture`),
                    //     updateImageToCloudinary($scope.fileSignatureCloudinary, `doctor_${doctorId}_signature_picture`)
                    // ]);

                    const doctorProfilePicturePromise = $scope.fileDoctorCloudinary.length > 0
                        ? updateImageToCloudinary($scope.fileDoctorCloudinary, `doctor_${doctorId}_profile_picture`)
                        : Promise.resolve();

                    const doctorSignaturePicturePromise = $scope.fileSignatureCloudinary.length > 0
                        ? updateImageToCloudinary($scope.fileSignatureCloudinary, `doctor_${doctorId}_signature_picture`)
                        : Promise.resolve();

                    Promise.all([
                        doctorProfilePicturePromise,
                        doctorSignaturePicturePromise
                    ])
