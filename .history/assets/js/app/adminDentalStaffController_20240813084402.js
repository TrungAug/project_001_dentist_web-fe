app.controller('AdminDentalStaffController', function ($scope, $http, $rootScope, $location, $timeout, API, adminBreadcrumbService) {
    ...
    $scope.crudDentalStaff = () => {
        ...
        $scope.createdentalStaff = () => {
            ...
            if (valid) {
                $scope.formDentalStaff.imageURL = $scope.filenames[0]
    ...
    $scope.listImg = function () {
        $http.get(url + '/listImage/imgStaff', { headers: headers }).then(response => {
    ...
    $scope.deleteImg = (filename) => {
        $http.delete(url + "/deleteImage/imgStaff/" + filename, { headers: headers }).then(resp => {
