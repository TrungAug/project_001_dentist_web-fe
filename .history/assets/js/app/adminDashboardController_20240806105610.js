app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.generateMMYYYYData = () => {
    $http.get(url + '/get-date-data').then(response => {
      //$scope.dateDataDB = response.data
  ...
  $scope.resetMonth = () => {
    ...
    $scope.formFilter.id = $scope.dateDataDB.find(item => item === $scope.filterByMonth)
  }
  ...
  $scope.initUI = () => {
    ...
    $('#filterByMonth').on('change', function () {
      $timeout(function () {
       ...
       console.log("$scope.dateDataDB",$scope.dateDataDB);
       
         $scope.filterByMonth=$scope.dateDataDB.find(item => item.id === selectedVals)
        // console.log("$scope.filterByMonth",$scope.filterByMonth);
