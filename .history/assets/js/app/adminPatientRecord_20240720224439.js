...
app.controller('AdminPatientRecord', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.exportPDF = () => {
        ...
        // Delay to ensure content is fully loaded/rendered
        setTimeout(() => {
            html2canvas(pdfContent, {
                useCORS: true,
                allowTaint: false,
                ...
                if (canvas.width === 0 || canvas.height === 0) {
                    console.error("Canvas size is zero, indicating an issue with rendering");
                    return;
                }
                ...
                if (imgData === 'data:,') {
                    console.error("Generated imgData is empty, indicating an issue with rendering");
                    return;
                }
