app.controller('CustomerRecoredController', function ($scope, $http, $rootScope, $location, $timeout, $window, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.getListAppointmentStatus = () => {
        $http.get(url + '/appointment-status', { headers: headers }).then(resp => {
           return resp.data.find((item) => item.status.toLowerCase() === 'đã hủy').appointment_StatusId
        })
    }
    ...
    $scope.cancelAppoinmnet = (appointment) => {
        ...
        Promise.all([duRequest, appServiceRequest]).then(([duResponse, appServiceResponse]) => {
            ...
            // let appDeleteRequest=$http.put(url + `/soft-delete-appointment/${appId}`, angular.toJson(dataApp),{ headers: headers })
