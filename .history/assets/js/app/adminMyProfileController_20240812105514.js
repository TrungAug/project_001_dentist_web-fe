app.controller('AdminMyProfileController', function ($scope, $http, $rootScope, $location, $timeout, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.patternPassword = (password) => {
        ...
        spanNewPassword.style.top = $scope.isFocusPassword && !$scope.isPatternPassword ? '39%' : '42%'

        const spanConfirmPassword= document.getElementById('spanConfirmPassword')
        spanConfirmPassword.style.top = $scope.isFocusPassword && !$scope.isPatternPassword ? '72%' : '70%'

        $scope.checkSamePreviousPassword(password)
    }

    $scope.checkSamePreviousPassword=(newPassword)=>{
        let passwordMap={
            newPassword:newPassword,
            currentPassword:$scope.account.password
        }
        // $scope.isSamePreviousPassword=false
        console.log("aaaa",angular.toJson(passwordMap));
        let dataJson=angular.toJson(passwordMap)
        $http.post(url+'',dataJson,{headers:headers}).then(response=>{
