app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route, API, adminBreadcrumbService) {
    ...
    $scope.detailByShift = (shiftId) => {
        ...
        let dsByShift = [];
        let doctorSet = new Set(); // Sử dụng Set để theo dõi các bác sĩ đã thêm vào
        ...
        for (let key in $scope.listDoctorScheduleByTimeRangesDB) {
            ...
            filteredArr.forEach(ds => {
                // Kiểm tra nếu bác sĩ chưa có trong Set
                if (!doctorSet.has(ds.doctorId)) {
                    dsByShift.push(ds);
                    doctorSet.add(ds.doctorId); // Thêm bác sĩ vào Set để theo dõi
                }
            });
        }
    
        console.log("dsByShift", dsByShift); // In kết quả ra console để kiểm tra
