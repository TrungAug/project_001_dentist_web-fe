...
app.controller('AdminPatientRecord', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.exportPDF = () => {
        ...
        // Log the dimensions and visibility of the element
        console.log("Element dimensions:", pdfContent.offsetWidth, "x", pdfContent.offsetHeight);
        console.log("Element visibility:", window.getComputedStyle(pdfContent).display);
    
        // Ensure the element is visible and has dimensions
        if (pdfContent.offsetWidth === 0 || pdfContent.offsetHeight === 0) {
            console.error('Element has no dimensions. Make sure it is visible.');
            return;
        }
