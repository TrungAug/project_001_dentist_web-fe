  ...
  $scope.fetchAppointments = (param, targetScope) => {
    ...
    return $http.get(url + '/appoinment-by-day-month-year', { params: param, headers: headers })
      .then(resp => {
        ...
        if (Array.isArray(data)) {
          if (data.length > 0) {
            ...
            $scope[targetScope] = []; // Hoặc giá trị mặc định khác
          }
        } else {
          console.error('Invalid data received: Expected an array but got:', data);
      ...
      .catch(error => {
        ...
        // Xử lý lỗi, có thể gán giá trị mặc định hoặc thông báo lỗi cho người dùng
        $scope[targetScope] = []; // Hoặc giá trị mặc định khác
      });
  }
