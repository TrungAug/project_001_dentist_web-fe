...
app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService) {
  ...
  $scope.getRevenue = (today) => {
    ...
    $http.get(url + '/get-revenue', { params: p }).then(response => {
      $scope.revenueByDay = response.data
      console.log("$scope.revenueByDay",$scope.revenueByDay==0);
      
    })
  }
  
  $scope.getRevenue($scope.today)
