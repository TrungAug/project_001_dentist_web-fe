app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchListAppointmentByYear = async (dateString) => {
    ...
    let respCurrentYear = await $http.get(url + '/appoinment-by-day-month-year', { params: param, headers: headers })
    $scope.listAppointmentCurrentYearDB = respCurrentYear.data
    let currentYearData=respCurrentYear.data

    let respPreviousYear= await $http.get(url + '/appoinment-by-day-month-year', { params: param, headers: headers })
