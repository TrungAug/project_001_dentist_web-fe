...
app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService) {
    ...
    $('.select2').select2(
      ...
      $('#filterByMoth').on('change', function () {
        $timeout(function () {
            ...
            $scope.filterByMoth = processSelect2Service.processSelect2Data(selectedVals)
            console.log("$scope.filterByMoth",$scope.filterByMoth);
  ...
  $scope.initUI()
  $scope.getListAppointment()
  $scope.chartArea(moment(new Date()).format('MM-YYYY'))
  $scope.generateMMYYYYData()
})
