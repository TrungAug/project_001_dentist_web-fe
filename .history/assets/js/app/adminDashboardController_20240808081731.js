app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchAppointments = (dateString, targetScope) => {
    ...
    // })
    return $scope.getAppointmentsByDateMonthYear(dateString).then(resp => {
      $scope[targetScope] = resp.data;
  ...
  $scope.fetchListAppointmentByYear = (dateString) => {
    ...
    // $scope.fetchAppointments(dateString, 'listAppointmentCurrentYearDB')
    // $scope.fetchAppointments(prevYear.toString(), 'listAppointmentPreviousYearDB')
    Promise.all([
      $scope.fetchAppointments(dateString, 'listAppointmentCurrentYearDB'),
      $scope.fetchAppointments(prevYear.toString(), 'listAppointmentPreviousYearDB')
    ]).then(([currentYearData, previousYearData]) => {
      console.log("Final Current Year Data:", $scope.listAppointmentCurrentYearDB);
      console.log("Final Previous Year Data:", $scope.listAppointmentPreviousYearDB);
    }).catch(error => {
  ...
  $scope.initPercentageByYear = (currentLength, previousLength) => {
  ...
  $scope.fetchListAppointmentByYear(moment().year().toString())
