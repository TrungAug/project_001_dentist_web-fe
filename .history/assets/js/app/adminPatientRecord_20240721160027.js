...
app.controller('AdminPatientRecord', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.exportPDF = () => {
        ...
        // Log computed styles of the element and its parents
        let element = pdfContent;
        while (element) {
            console.log("Element:", element.tagName, "ID:", element.id, "Class:", element.className);
        ...
        // Try to capture content as SVG
        const svgData = `
            <svg xmlns="http://www.w3.org/2000/svg" width="${pdfContent.offsetWidth}" height="${pdfContent.offsetHeight}">
