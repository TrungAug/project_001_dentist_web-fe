app.controller('AdminCalendar', function ($scope, $http, SocketService, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route, convertMoneyService, API, adminBreadcrumbService) {
    ...
    $scope.getDoctorName = function(appointment) {
        if (appointment.doctor) {
            return appointment.doctor.fullName;
        } else {
            // Return safe HTML string for the placeholder
            return $sce.trustAsHtml('<span class="badge bg-primary">Chờ sắp bác sĩ</span>');
        }
    };
