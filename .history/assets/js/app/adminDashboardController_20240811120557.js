app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchAppointmentsByCurrentYear = (param) => {
    $http.get(url + '/appoinment-by-day-month-year', { params: param, headers: headers })
      .then(resp => {
        $scope.listAppointmentCurrentYearDB = resp.data
      })
  }

  $scope.fetchAppointmentsByPreviousYear = (param) => {
    $http.get(url + '/appoinment-by-day-month-year', { params: param, headers: headers })
      .then(resp => {
        $scope.listAppointmentPreviousYearDB = resp.data
      })
  }
