...
app.controller('AdminPatientRecord', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getAllAppGroupByDate=(selectedDate,) => {
        let params={
            startDate: moment(selectedDate.split(' - ')[0], 'DD/MM/YYYY').format('YYYY-MM-DD'),
            endDate:moment(selectedDate.split(' - ')[1], 'DD/MM/YYYY').format('YYYY-MM-DD'),
            patientIds:"",
            doctorIds:""
        }
    }
