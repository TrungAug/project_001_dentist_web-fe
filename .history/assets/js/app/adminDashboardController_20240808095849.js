app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchTotalRevenue = (dateString) => {
  ...
  $scope.fetchRevenueByDateOrMonth = (dateString) => {
    let processParam = $scope.generateParamsForGetAppointments(dateString)
    let date =moment(dateString,'DD/MM/YYYY').format('YYYY-MM-DD')    
    let prevDate =moment(dateString,'DD/MM/YYYY').subtract(1, 'days').format('YYYY-MM-DD')    
    let param = {
      date: processParam.day?date:null,
      month: processParam.month,
      year: processParam.year
    };

    let prevParam = {
      date: processParam.day?prevDate:null,
      month: (processParam.month-1).toString(),
      year: processParam.year
    };
    console.log("param kuku",param);
    console.log("prevParam kaka",prevParam);
  ...
  $scope.initPercentageByYear = (currentLength, previousLength) => {
    ...
    $scope.percentageYear = percentage
    $scope.$apply();
  ...
  $scope.initUI = () => {
    ...
    $('#filterByMonth').on('change', function () {
      $timeout(function () {
        ...
        $scope.thisYear = year
        $scope.fetchTotalRevenue($scope.filterByMonth)
