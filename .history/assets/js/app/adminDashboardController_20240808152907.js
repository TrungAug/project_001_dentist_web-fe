app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchListAppointmentByYear = (dateString) => {
    ...
    Promise.all([
      ...
      $scope.completedAppointments = currentYearData.filter(item => item.appointmentStatus.status.toUpperCase() === "HOÀN THÀNH")
      console.log("$scope.completedAppointments",$scope.completedAppointments);
