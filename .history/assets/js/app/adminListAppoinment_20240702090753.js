app.controller('AdminListAppoinment', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route) {
    ...
    $scope.updateStatus = (app, statusId) => {
        var originalStatus = $scope.listAppointmentStatusBD.find(s => s.status === app.appointment.appointmentStatus.status)//status trong app đã thay đổi id nhưng status chưa thay đổi, dựa vào đây để lấy lại status cũ
