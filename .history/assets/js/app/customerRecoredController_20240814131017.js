    ...
    $scope.cancelAppoinmnet = (appointment) => {
        ...
        Promise.all([duRequest, appServiceRequest]).then(([duResponse, appServiceResponse]) => {
            ...
            duResponse.data.forEach(du => {
                ...
                duPutRequest.push($http.delete(url + `/soft-delete-doctorUnavailability/${id}`,{ headers: headers }))
            })
            let servicePutRequest = []
            appServiceResponse.data.forEach(appService => {
                let id = appService.appointment_ServiceId
                servicePutRequest.push($http.put(url + `/appointment-patient-record/${id}`, angular.toJson(appointmentPatientRecordRequest), { headers: headers }))
            })
        })
