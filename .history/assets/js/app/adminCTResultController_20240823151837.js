app.controller('AdminCTResultController', function ($scope, $http, $rootScope, $location, TimezoneService, $timeout, API, adminBreadcrumbService, processSelect2Service) {
        ...
        $scope.createCTResult = async () => {
            ...
            if (valid) {
                ...
                await Promise.all([...abnormalityRequest]).then(() => {
                    ...
                    Swal.fire({
                        title: "Thất bại!",
                        html: '<p class="text-danger">Thêm mới thất bại!</p>',
                        icon: "error"
                    })
                })
            }
    ...
    $scope.zoomInImage = () => {
        ...
        mousemove.addEventListener("mouseleave", (e) => {
    ...
    //         if (!mousemove || !imgmove) {
    //             console.error("Elements not found in the DOM");
    //             return;
    //         }

    //         // Remove existing event listeners if any
    //         mousemove.removeEventListener("mousemove", onMouseMove);
    //         mousemove.removeEventListener("mouseleave", onMouseLeave);

    //         const onMouseMove = (e) => {
    //             // Use offsetX and offsetY to get mouse position relative to the element
    //             const x = e.offsetX;
    //             const y = e.offsetY;

    //             // Set the transform origin and zoom level
    //             imgmove.style.transformOrigin = `${x}px ${y}px`;
    //             imgmove.style.transform = "scale(2)";
    //             imgmove.style.transition = "transform 0.3s ease"; // Optional smooth transition
    //         };
    ...
    //         mousemove.addEventListener("mouseleave", onMouseLeave);
    //     });
    // };
