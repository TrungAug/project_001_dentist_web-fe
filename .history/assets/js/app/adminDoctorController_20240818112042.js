        ...
        $scope.createDoctor = () => {
            ...
            if (valid) {
                ...
                $http.post(url + '/doctor', requsetDoctorJSON, { headers: headers }).then(respone => {
                    ...
                    for (var i = 0; i < $scope.fileDoctorCloudinary.length; i++) {
                        formData.append("file", $scope.fileDoctorCloudinary[i]);
                    }
                    ...
                    var publicIdSignature = 'doctor_' + respone.data.doctorId + '_signature_picture'
                    var formDataSignature = new FormData();
                    for (var i = 0; i < $scope.fileSignatureCloudinary.length; i++) {
                        formDataSignature.append("file", $scope.fileSignatureCloudinary[i]);
                    }
                    formDataSignature.append('publicId', publicIdSignature);
                    var requestSignatureCloundinary = $http.post(url + "/upload-image-cloudinary", formDataSignature, {
                        transformRequest: angular.identity,
                        headers: {
                            'Content-Type': undefined,
                            ...headers
                    ...
                    Promise.all([requestCloundinary, requestSignatureCloundinary])
                        .then(responses => {
                            $timeout(() => {
                                $scope.isLoadingCreate = false
                            }, 3000)
