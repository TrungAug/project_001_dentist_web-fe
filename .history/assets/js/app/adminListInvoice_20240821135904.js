app.controller('AdminListInvoice', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route, API, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.getAppointmentServiceByAppointmentId=(appointmentId) => {
        $http.get(url+'/appointment-service-appointment-id/'+appointmentId,{headers:headers}).then((response) => {
            ...
            console.log("$scope.listAppointmentService",$scope.listAppointmentService);
    ...
    $scope.getAllBillExceptDeleted=() => {
        $http.get(url+'/bill-except-deleted',{headers:headers}).then((response) => {
            $scope.listBill=response.data
            console.log("$scope.listBill",$scope.listBill);
        ...
        $http.post(url+"/bill",angular.toJson(billRequest),{headers:headers}).then(response=>{
            $timeout(()=>{
                Swal.fire({
                    position:'top-end',
                    title: "Thông báo!",
                    html: '<p>Tạo hóa đơn thành công!</p>',
                    icon: "success"
                })
                $scope.initData()
                $route.reload()
