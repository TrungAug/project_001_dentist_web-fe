app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchListAppointmentByYear = async (dateString) => {
    ...
    const fetchData = async (param) => {
      try {
        const params = angular.toJson(param)
    ...
    const paramCurrentYear = await $scope.generateParamsForGetData(dateString);
    const prevYear = parseInt(paramCurrentYear.year) - 1;
    const paramPreviousYear = await $scope.generateParamsForGetData(prevYear.toString());
    console.log("paramPreviousYear",paramPreviousYear);
    

    const [currentYearData, previousYearData] = await Promise.all([
      fetchData(paramCurrentYear),
      fetchData(paramPreviousYear)
    ]);
