app.controller('AdminCreateAccountController', function ($scope, $http, $rootScope, $location, $timeout, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.createAccount = () => {
        ...
        $timeout(() => {
            if (checkForm) {
                console.log("angular.toJson($scope.accForm)",angular.toJson($scope.accForm));
    ...
    $scope.getAccountName = (account) => {
        ...
        switch (roleName) {
            case 'BENH_NHAN':
                return account.patient.fullName
            case 'BAC_SI':
                return account.doctor.fullName
            case 'NHAN_VIEN':
            case 'HANH_CHINH':
            case 'KY_THUAT':
            case 'ADMIN':
            case 'LE_TAN':
            case 'QUAN_LY':
                return account.dentalStaff.fullname
            default:
                return null
        }
    ...
    $scope.disabledAccount = (account) => {
        ...
        Swal.fire({
            ...
            icon: (account.activity ? "primary" : "warnig"),
            ...
            if (rs.isConfirmed) {
                ...
                $timeout(() => {
                    $http.post(url + "/disabled-user", angular.toJson(userRequest), { headers: headers }).then(response => {
                        Swal.fire({
                            title: "Thành công!",
                            html: (account.activity ? "Kích hoạt " : "Hủy ") + "tài khoản thành công!",
                            icon: "success"
                        })
                    })
                }, 3000)
