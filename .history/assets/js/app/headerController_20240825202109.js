app.controller('HeaderController', function ($scope, $http, $rootScope, SocketService, $location, $timeout, $window, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    // $scope.isQL = API.getUser() ? API.getUser().split("-")[1] == 'QUAN_LY' : null
    // $scope.isHC = API.getUser() ? API.getUser().split("-")[1] == 'HANH_CHINH' : null
    // $scope.isKT = API.getUser() ? API.getUser().split("-")[1] == 'KY_THUAT' : null
    // $scope.isBS = API.getUser() ? API.getUser().split("-")[1] == 'BAC_SI' : null
