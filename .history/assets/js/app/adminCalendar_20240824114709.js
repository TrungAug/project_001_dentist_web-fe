app.controller('AdminCalendar', function ($scope, $http, SocketService, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route, convertMoneyService, API, adminBreadcrumbService) {
    ...
    $scope.getListAppointmentStatusFilter = () => {
        $http.get(url + '/appointment-status-except-deleted', { headers: headers }).then(resp => {
            ...
            // $scope.listAppointmentStatusBDFilter = [allOptions].concat(resp.data);
    ...
    $scope.updateAppointment = () => {
        ...
        console.log("originalDu", originalDu);
        console.log("reqDu", reqDu);
        ...
        return new Promise((resolve, reject) => {
            $http.get(url + '/doctorUnavailability', { headers: headers }).then((response) => {
                ...
                if ($scope.selectedStatusLApp.length > 0) {
                    // if($scope.selectedStatusLApp[0]==-1){
                    //     filteredData = response.data
                    // }       
                    console.log("$scope.selectedStatusLApp[0]", $scope.selectedStatusLApp[0]);

                    console.log("$scope.selectedStatusLApp[0]==-1", $scope.selectedStatusLApp[0] == -1);

                    filteredData = filteredData.filter(item => {
                        return $scope.selectedStatusLApp.includes(item.appointment.appointmentStatus.appointment_StatusId
                        );
                    });
                }
    ...
    $scope.getPrescriptionWithMedicinesByAppointment = (appointmentId) => {
        ...
        $http.get(url + '/prescription-by-appointment', { params: params, headers: headers }).then(response => {
            ...
            let medicinesList = $scope.listPrescriptionByAppointmentAndMedicinesDB[0].medicinesList;
            let servicesString = "TOOTH TEETH " + medicinesList.map(medicine => medicine.medicinesId).join(' ')
            $http.post(url + '/generateBarcode', { text: servicesString }, { headers: headers })
                .then(response => {
    ...
    $scope.initializeUIComponentsLApp = () => {
        ...
        $('#appStatusFilterTableApp').on('change', function () {
            $timeout(function () {
                ...
                console.log("$scope.selectedStatusLApp",$scope.selectedStatusLApp);
