app.controller('AdminCalendar', function ($scope, $http, SocketService, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route, convertMoneyService, API, adminBreadcrumbService) {
    ...
    $scope.saveAppointment = async () => {
        ...
        if (valid) {
            try {
                ...
                Swal.fire({
                    title: "Lỗi!",
                    html: "Có lỗi xảy ra khi đặt lịch khám. Vui lòng thử lại.",
                    icon: "error"
                });
            }
        }
    ...
    $scope.getNameStatus = (statusId) => {
        ...
        if (!st) {
            console.log("Không tìm thấy trạng thái với id:", statusId);
            return 'trạng thái không xác định'; // Return a default message or value if status is not found
        }

        return st.status.toLowerCase(); // Return the found status
    }
    ...
    $scope.updateAppointment = () => {
        ...
        // send to that user alert
        $http.get(url + '/get-email-by-patient-id', {
            params: { patientId: reqApp.patientId },
            ...
            let message = {
                getterMail: email,
                body: "Lịch khám ngày " + reqApp.appointmentDate.split("T")[0] + " " + $scope.getNameStatus(reqApp.appointmentStatus)
            };
            console.log(message);
        ...
        Promise.all([
            ...
            Swal.fire({
                title: "Thành công!",
                html: "Cập nhật lịch khám thành công",
                icon: "success"
            }).then(rs => {
                const btnCloseFormUpApp = document.getElementById('btn-close-formUpApp')
                btnCloseFormUpApp.click()
    ...
    $scope.getListDoctorUnavailabilityAllDoctor = () => {
        $scope.processDoctorUnavailabilityAllDoctor().then(result => {
            ...
            return $http.get(url + '/appointment', { headers: headers })
        }).then(resp => {
            let appointments = resp.data.filter(appointment => {
                return appointment.doctor === null;
            });
            $scope.listAppointmentDBFromDu = $scope.listAppointmentDBFromDu.concat(appointments);
            console.log("$scope.listAppointmentDBFromDu cc",$scope.listAppointmentDBFromDu);
            // $scope.listAppointmentDBFromDu.sort(function(a, b) {               
            //     var dateA = new Date(a.appointmentDate);
            //     var dateB = new Date(b.appointmentDate);
            //     return dateB - dateA;
            // });
            $scope.listAppointmentDBFromDu.sort(function(a, b) {
                // First, compare by appointmentDate in descending order
                var dateA = new Date(a.appointmentDate);
                var dateB = new Date(b.appointmentDate);
                if (dateA > dateB) return -1;  // dateB comes before dateA
                if (dateA < dateB) return 1;   // dateA comes before dateB
                
                // If dates are equal, compare by appointmentId in descending order
                var idA = a.appointmentId;
                var idB = b.appointmentId;
                if (idA > idB) return -1;  // idB comes before idA
                if (idA < idB) return 1;   // idA comes before idB
                
                return 0; // Dates and IDs are equal
            });
            $timeout(() => {
    ...
    $scope.getBillByAppointment = (appointmentId, listBillByAppointmentAndPatientDB) => {
        ...
        let bill = listBillByAppointmentAndPatientDB.filter(bill => bill.appointments.appointmentId === appointmentId)
        return bill
    }
    ...
    $scope.clickAppointmentToAccept = () => {
        ...
        console.log("click " + appointmentId);

        // Define a function to attempt clicking the element
        const attemptClick = (retries) => {
            if (retries <= 0) {
                // No more retries left, just clear localStorage
                localStorage.removeItem('clickedAppointmentId');
                return;
            }

            // Try to find the <p> element with the corresponding id
            const pElement = document.getElementById('appointment' + appointmentId);
            console.log("Attempting to click " + appointmentId);

            if (pElement) {
                // Click the element if found
                pElement.click();
                console.log("Clicked element with id " + 'appointment' + appointmentId);
                // Successfully clicked, remove from localStorage
                localStorage.removeItem('clickedAppointmentId');
            } else {
                // Element not found, wait for a short delay and retry
                $timeout(() => attemptClick(retries - 1), 500); // Retry after 500ms
            }
        };

        // Start attempting to click with a certain number of retries
        attemptClick(10); // Number of retries
    };
