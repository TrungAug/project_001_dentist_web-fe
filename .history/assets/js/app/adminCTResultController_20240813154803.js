    ...
    $scope.crudCTResult = () => {
        ...
        $scope.updateCTResult = () => {
            ...
            if (valid) {
                ...
                // Tạo các Promise cho các yêu cầu HTTP
                var updateCTResultPromise = $http.put(url + '/appointment-ct-result/' + appointmentCTResultId, requestCTResultJSON, { headers: headers });
                var updateAbnormalityPromise = $http.put(url + '/ct-result-abnormality/' + $scope.formCTResultAbnormality.ctresultAbnormalityId, requestCTResultAbnormalityJSON, { headers: headers });

                // Sử dụng Promise.all để chờ tất cả các Promise hoàn thành
                Promise.all([updateCTResultPromise, updateAbnormalityPromise])
                    .then(([responseCTResult, responseAbnormality]) => {
                        $timeout(() => {
                            $scope.isLoadingUpdate = false;
                        }, 3000);    
                    })
