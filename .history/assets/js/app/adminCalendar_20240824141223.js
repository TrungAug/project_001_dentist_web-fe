app.controller('AdminCalendar', function ($scope, $http, SocketService, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route, convertMoneyService, API, adminBreadcrumbService) {
    ...
    $scope.getAllDentalIssuesExceptDeleted = () => {
        $http.get(url + '/dental-issues-except-deleted', { headers: headers }).then(response => {
            // let allOptions={
            //     dentalIssuesId:null,
            //     name: '---Nhập triệu chứng ---'
            // }
            // $scope.listDentalIssueDB = response.data
            // $scope.listDentalIssueDB.unshift(allOptions)
            ...
            let allOptions = [
                ...
                ...response.data.map(issue => ({
                    id: issue.dentalIssuesId,
                    text: issue.name
                }))
            ];
            // Update Select2 with new options
            $('.select2-multi-dentalIssue').empty().select2({
                data: allOptions,
                multiple: true,
                theme: 'bootstrap4',
    ...
    $scope.initializeUIComponents = () => {
        ...
        $timeout(function () {
            ...
            $('.select2-multi-dentalIssue').val(null).trigger('change')
        }, 100)
        ...
        $('#appointmentType').on('change', function () {
            $timeout(function () {
                ...
                console.log("$scope.formApp.appointmentTypeId",$scope.formApp.appointmentTypeId);
