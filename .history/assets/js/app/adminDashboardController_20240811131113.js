app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  //   const prevYear = currentYear - 1;
  //   let currentParam =await $scope.generateParamsForGetData(currentYear.toString())
  //   let prevParam = await $scope.generateParamsForGetData(prevYear.toString())

  //   let respCurrentYear = await $http.get(url + '/appoinment-by-day-month-year', { params: currentParam, headers: headers })
  //   console.log("respCurrentYear.data",respCurrentYear.data);
    
  //   $scope.listAppointmentCurrentYearDB = respCurrentYear.data
  //   let currentYearData=respCurrentYear.data

  //   let respPreviousYear= await $http.get(url + '/appoinment-by-day-month-year', { params: prevParam, headers: headers })
  //   console.log("respPreviousYear.data",respPreviousYear.data);
    
  //   $scope.listAppointmentPreviousYearDB = respPreviousYear.data
  //   let previousYearData= respPreviousYear.data
  ...
  $scope.fetchListAppointmentByYear = async (dateString) => {
    ...
    const fetchData = async (year) => {
      ...
      const response = await $http.get(`${url}/appoinment-by-day-month-year`, { params, headers });
      return response.data;
    };
  
 
    const currentYear = parseInt((await $scope.generateParamsForGetData(dateString)).year);
    const prevYear = currentYear - 1;
  ...
  $scope.fetchListAppointmentByMonth = async (dateString) => {
    ...
    let prevParam = {}
    if (param.day != null) {
      prevParam = await $scope.generateParamsForGetData(moment(dateString, 'DD/MM/YYYY').subtract(1, 'days').format('DD/MM/YYYY'))
    } else if (param.month != null) {
      let prevDateString = (param.month - 1) + "/" + param.year
      prevParam = await $scope.generateParamsForGetData(prevDateString)
    }
    let currentMonthResponse = await $http.get(url + '/appoinment-by-day-month-year', { params: param, headers: headers }) 
    $scope.listAppointmentDBTodayOrThisMonth=currentMonthResponse.data
    currentMonthData=currentMonthResponse.data

    let previousMonthResponse=await $http.get(url + '/appoinment-by-day-month-year', { params: prevParam, headers: headers }) 
    $scope.listAppointmentDBPreviousDayOrPreviousMonth=previousMonthResponse.data
    previousMonthData=previousMonthResponse.data
