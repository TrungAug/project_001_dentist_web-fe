app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.getRevenueGrowthComparison = (date, month,year) => {
    ...
    if (date && $scope.venueToday !== undefined && $scope.venuePreviousToday !== undefined) {
      let isMonth = false
      let isYear=false
      updateGrowthData($scope.venueToday, $scope.venuePreviousToday, isMonth,isYear);
    }

    if (month && $scope.venueByMonth !== undefined && $scope.revenuePreviousMonth !== undefined) {
      let isMonth = true
      let isYear=false
      updateGrowthData($scope.venueByMonth, $scope.revenuePreviousMonth, isMonth,isYear);
    }

    if (year && $scope.venueByMonth !== undefined && $scope.revenuePreviousMonth !== undefined) {
      let isMonth = true
      updateGrowthData($scope.venueTotal, $scope.venuePrviousYear, isMonth);
    }
