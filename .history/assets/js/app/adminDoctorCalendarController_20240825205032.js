    ...
    $scope.getCTResultAbnormalityByAppointment = (appointmentId) => {
        $http.get(url + "/ct-result-abnormality-by-appointmnet-id/" + appointmentId, { headers: headers }).then((response) => {
            ...
            response.data.forEach(rs => {
                ...
                if (!resultMap[appointmentCTResult]) {
                    resultMap[appointmentCTResult] = [];
                }

                resultMap[appointmentCTResult].push(abnormality);
            });

            $scope.listCTResultAbnormalityByAppointment = Object.keys(resultMap).map(appointmentCTResult => {
                return [appointmentCTResult, resultMap[appointmentCTResult]];
            });
