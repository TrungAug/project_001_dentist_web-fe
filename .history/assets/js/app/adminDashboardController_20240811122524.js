app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchAppointments = (param, targetScope) => {
    // return $http.get(url + '/appoinment-by-day-month-year', { params: param, headers: headers }).then(resp => {
    //   console.log(resp.data);
    //   $scope[targetScope] = resp.data
    //   return $scope[targetScope]
    // })
    console.log('Fetching appointments with params:', param);

    return $http.get(url + '/appoinment-by-day-month-year', { params: param, headers: headers })
      .then(resp => {
        let data = resp.data;
        console.log('Response data:', data);

        // Kiểm tra xem dữ liệu có phải là mảng không và có thể sử dụng được không
        if (data && Array.isArray(data)) {
          if (data.length > 0) {
            $scope[targetScope] = data;
          } else {
            console.warn('Received an empty array:', data);
            $scope[targetScope] = [];
          }
        } else {
      ...
      .catch(error => {
        ...
        console.error('Error details:', error.data ? error.data : 'No response data');
        $scope[targetScope] = [];
      });
  ...
  $scope.fetchListAppointmentByYear = async (dateString) => {
    ...
    //   if (currentYearData == null || previousYearData == null) return
    //   const cancelStatus = ["ĐÃ HỦY", "KHÔNG ĐẾN", "HOÃN"]
    //   $scope.listCompletedAppointments = currentYearData.filter(item => item.appointmentStatus.status.toUpperCase() === "HOÀN THÀNH")
    //   $scope.listCancelsAppointments = currentYearData.filter(item => cancelStatus.includes(item.appointmentStatus.status.toUpperCase()))
  ...
  $scope.fetchListAppointmentByMonth = async (dateString) => {
    ...
    //   if (currentMonthData == null || previousMonthData == null) return
    //   $scope.initPercentageAppoinmentByMonth(currentMonthData.length, previousMonthData.length)
    // }).catch(error => {
    //   console.error("Lỗi lấy dữ liệu:", error);
    // });
