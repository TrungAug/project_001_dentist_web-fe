app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route, convertMoneyService) {
    ...
    $scope.getPrescriptionByAppointment = (appointment,listPrescriptionByAppointmentAndMedicinesDB) => {
        ...
        listPrescriptionByAppointmentAndMedicinesDB.forEach(item => {
            console.log("itiem",item.prescription.appointmentPatientRecord.appointmentPatientRecordId);
        })
        // let prescription = listPrescriptionByAppointmentAndMedicinesDB.filter(item => item.appointmentPatientRecord.appointmentPatientRecordId===aprId)
