app.controller('AdminListInvoice', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route,API,adminBreadcrumbService) {
    ...
    $scope.getAllAppointmentStatusExceptDeleted=()=>{
        $http.get(url+"/appointment-status-except-deleted",{headers:headers}).then(response=>{
            let completedStatus=response.data.filter(st=>st.status.toLowerCase()=='hoàn thành').$scope.$scope.completedStatusId=completedStatus.appointment_StatusId
            console.log("$scope.completedStatusId",$scope.completedStatusId);
