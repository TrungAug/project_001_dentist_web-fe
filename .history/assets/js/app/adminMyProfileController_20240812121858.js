app.controller('AdminMyProfileController', function ($scope, $http, $rootScope, $location, $timeout, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.initData = () => {
        ...
        $scope.isSamePreviousPasswordNew = false
        $scope.isSamePreviousPasswordCurrent = false
    ...
    $scope.checkNewPasswordInput = (password) => {
        ...
        const spanNewPassword = document.getElementById('spanNewPassword')
        spanNewPassword.style.top = $scope.isFocusNewPassword && !$scope.isPatternPassword ? '39%' : '42%'

        const spanConfirmPassword = document.getElementById('spanConfirmPassword')
        spanConfirmPassword.style.top = $scope.isFocusNewPassword && !$scope.isPatternPassword ? '72%' : '70%'
    ...
    $scope.checkSamePreviousPassword = (inpPassword,field) => {
        ...
        $http.post(url + '/valid-password', dataJson, { headers: headers }).then(response => {
            if(field==='Current'){
                $scope.isSamePreviousPasswordCurrent = response.data
            }else if(field==='New'){
                $scope.isSamePreviousPasswordNew = response.data
            }
            //$scope.isSamePreviousPassword = response.data
            //console.log("$scope.isSamePreviousPassword[field]",$scope.isSamePreviousPassword[field]);
    ...
    $scope.checkConfirmPasswordInput=(inpPassword)=>{
        ...
        console.log("$scope.isMatchNewPassword",$scope.isMatchNewPassword);
        
        
    }

    $scope.toggleBlur = (field) => {
        // if (field === 'current'&&!$scope.isSamePreviousPassword) {
        //     $scope.isFocusCurrentPassword = false
        // } else if (field === 'new'&&$scope.isSamePreviousPassword) {
        //     $scope.isFocusNewPassword = false
        // } else if (field === 'confirm'&&!$scope.isMatchNewPassword) {
        //     $scope.isFocusConfirmPassword = false
        // }
    }

    $scope.changePassword = () => {
        let validCurrentPassword = !$scope.isSamePreviousPassword
        let validNewPassword = !$scope.isPatternPassword&&$scope.isSamePreviousPassword
        let validConfirmPassword = !$scope.isMatchNewPassword
        if(validCurrentPassword||validNewPassword||validConfirmPassword) return
        console.log("$scope.formChangePassword", $scope.formChangePassword);
    }
