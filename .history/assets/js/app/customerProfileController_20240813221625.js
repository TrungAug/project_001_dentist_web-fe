app.controller('CustomerProfileController', function ($scope, $http, $rootScope, $location, $timeout, $window, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('.drgpicker').on('apply.daterangepicker', function (ev, picker) {
            ...
            $scope.formPatient.birthday = new Date(picker.startDate)
        });
    ...
    $scope.deleteImg = (filename) => {
        $http.delete(url + "/deleteImage/imgPatient/" + filename, { headers: headers }).then(resp => {
    ...
    $scope.updateCustomer = () => {
        ...
        if ($scope.filenames[0] == null) {
            ...
            $scope.formPatient.imageURL = $scope.filenames[0]
        }
        let requsetCustomerJSON = angular.toJson($scope.formPatient)
        console.log("requsetCustomerJSON",requsetCustomerJSON);
        ...
        //         $http.post(url + '/move/tempFolder/imgPatient', requsetFileJSON, {
        //             transformRequest: angular.identity,
        //             headers: {
        //                 'Content-Type': undefined,
        //                 ...headers
