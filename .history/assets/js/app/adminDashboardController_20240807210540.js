app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchTotalRevenue = () => {
    $scope.getRevenue(null, null).then(response => {
  ...
  $scope.fetchRevenueByDateOrMonth = (date, month, year) => {
    $scope.getRevenue(date, month, year).then(response => {
      if (date) {
      ...
      $scope.fetchRevenueByPreviousDateOrPreviousMonth(date, month, year)
    }).catch(error => {
      console.error('Lỗi khi lấy dữ liệu:', error);
    });
  };
  ...
  $scope.fetchRevenueByPreviousDateOrPreviousMonth = (date, month, year) => {
    let previousDate = date ? moment(date).subtract(1, 'days').format('YYYY-MM-DD') : null
    let previousMonth = month ? (month - 1) : null
    $scope.getRevenue(previousDate, previousMonth, year).then(response => {
      if (date) {
      ...
      $scope.getRevenueGrowthComparison(date, month)
    }).catch(error => {
      console.error('Lỗi khi lấy dữ liệu:', error);
    });
  };

  $scope.getRevenue = (today, month, year) => {
    let params = {
      date: today,
      month: month,
      year: year
  ...
  $scope.getRevenueGrowthComparison = (date, month) => {
    ...
    const updateGrowthData = (currentRevenue, previousRevenue, isMonth) => {
      ...
      if (isMonth) {
        ...
        $scope.growthPercentage = growthPercentage !== 0 && previousRevenue !== 0
          ? growthPercentage.toFixed(2)
          : growthPercentage;
      }
    ...
    if (date && $scope.venueToday !== undefined && $scope.venuePreviousToday !== undefined) {
      let isMonth = false
      updateGrowthData($scope.venueToday, $scope.venuePreviousToday, isMonth);
    }

    if (month && $scope.venueByMonth !== undefined && $scope.revenuePreviousMonth !== undefined) {
      let isMonth = true
      updateGrowthData($scope.venueByMonth, $scope.revenuePreviousMonth, isMonth);
    }
  };
  ...
  $scope.getAppointmentsByDateMonthYear = (dateString) => {
    // dateString format DD/MM/YYYY
    let params = $scope.generateParamsForGetAppointments(dateString)
    return $http.get(url + '/appoinment-by-day-month-year', { params: params });
  }

  $scope.fetchAppointments = (dateString, targetScope) => {
    $scope.getAppointmentsByDateMonthYear(dateString).then(resp => {
  ...
  $scope.fetchListAppointmentByYear = async (dateString) => {
    // let prevDateString = (parseInt(dateString) - 1) + ""
    // await $scope.fetchAppointments(dateString, 'listAppointmentCurrentYearDB');
    // await $scope.fetchAppointments(prevDateString, 'listAppointmentPreviousYearDB');

    try {
      const currentYear = parseInt(dateString);
      const prevYear = currentYear - 1;

      await $scope.fetchAppointments(dateString, 'listAppointmentCurrentYearDB');
      await $scope.fetchAppointments(prevYear.toString(), 'listAppointmentPreviousYearDB');

      const currentLength = $scope.listAppointmentCurrentYearDB ? $scope.listAppointmentCurrentYearDB.length : 0;
      const previousLength = $scope.listAppointmentPreviousYearDB ? $scope.listAppointmentPreviousYearDB.length : 0;
      console.log("currentLength",currentLength);
      
      const percentage = $scope.calculateGrowthPercentage(currentLength, previousLength);
  ...
  $scope.fetchListAppointmentByToDay = (dateString) => {
    $scope.fetchAppointments(dateString, 'listAppointmentDBToday');
  }

  $scope.fetchListAppointmentByPreviousDay = (dateString) => {
    $scope.fetchAppointments(dateString, 'listAppointmentDBPreviousDay');
  }
  ...
  $scope.initUI = () => {
    ...
    $('#filterByMonth').on('change', function () {
      $timeout(function () {
        ...
        let month = parseInt($scope.filterByMonth.split('/')[0])
        let year = parseInt($scope.filterByMonth.split('/')[1])
        $scope.fetchRevenueByDateOrMonth(null, month, year);
      });
    });
  }
  ...
  $scope.fetchListAppointmentByYear(moment().year() + "")
  $scope.fetchListAppointmentByToDay(moment(new Date()).format('DD/MM/YYYY'))
  $scope.fetchListAppointmentByPreviousDay(moment(new Date()).subtract(1, 'days').format('DD/MM/YYYY'))
  $scope.chartArea($scope.filterByMonth)
  $scope.generateMMYYYYData()
})
