app.controller('BookingController', function ($scope, $http,SocketService ,$rootScope, $location, $timeout,API, $q) {
    ...
    console.log("$scope.isPatient",$scope.isPatient);
    ...
    $scope.init = function (){
        ...
        var savedServiceId = localStorage.getItem('selectedServiceId');
        
        // Nếu ID tồn tại và hợp lệ, thực hiện chức năng
        if (savedServiceId) {
            $scope.selectDoctor(savedServiceId);
            // Xóa ID khỏi localStorage sau khi sử dụng, nếu cần
    ...
    $scope.formValidateColor = function() {
        ...
        if($scope.isFastBookingForm){
            ...
            if(!$scope.showformShift){
                // console.log("Không chọn buổi khám");
            }else{
           ...
           //Nếu là form đặt hẹn với bác sĩ 
                ...
                if (shiftId) {
                    $http.get(url + '/time-of-shift-by-shift-id/' + shiftId, { headers: headers }).then(response => {
                        ...
                        angular.forEach($scope.listShiftHours, function(hour) {
                            if ($scope.listDoctorUnavailabilityByDoctorDB.some(avail => avail.timeOfShift.timeOfShiftId === hour.timeOfShiftId)) {
                                hour.isValid = true;
                            }
                        });
                        // console.log($scope.listShiftHours);
