app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchListAppointmentByYear = (dateString) => {
    $scope.fetchAppointments(dateString, 'listAppointmentDB');
  }
  ...
  $scope.calculateGrowthPercentage = (todayLength,pastLength) => {
    // let caculator = ((todayLength - pastLength) / (pastLength == 0 ? 1 : pastLength)) * 100
    // $scope.appFluctuate = Math.round(caculator);
    // $scope.appFluctuate = parseFloat($scope.appFluctuate.toFixed(2))
     // Ensure inputs are numbers
     todayLength = parseFloat(todayLength);
     pastLength = parseFloat(pastLength);
 
     if (isNaN(todayLength) || isNaN(pastLength)) {
         $scope.appFluctuate = 0; 
         return;
     }

     if (pastLength === 0) {
         $scope.appFluctuate = todayLength === 0 ? 0 : 100; 
     } else {
         let growth = ((todayLength - pastLength) / pastLength) * 100;
         $scope.appFluctuate = parseFloat(growth.toFixed(2));
     }
  }
