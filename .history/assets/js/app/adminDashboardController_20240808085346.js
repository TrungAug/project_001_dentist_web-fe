app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchListAppointmentByToDay = (dateString) => {
    ...
    let prevParam={}
    if(param.day!=''){
      prevParam=$scope.generateParamsForGetAppointments(moment(dateString).subtract(1, 'days').format('DD/MM/YYYY'))
    }else if(param.month!=''){
      let prevDateString=(parseInt(param.month)-1)+"/"+param.year
      prevParam=$scope.generateParamsForGetAppointments(prevDateString)
    }
    
    Promise.all([
      $scope.fetchAppointments(param, 'listAppointmentDBToday'),
      $scope.fetchAppointments(prevParam, 'listAppointmentDBPreviousDay')
    ])
  }
  ...
  $scope.initUI = () => {
    ...
    $('#filterByMonth').on('change', function () {
      $timeout(function () {
        ...
        // $scope.fetchListAppointmentByPreviousDay($scope.filterByMonth)
      });
    });
  }
