app.controller('AdminListInvoice', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route, API, adminBreadcrumbService) {
    ...
    $scope.getAllAppointmentStatusExceptDeleted = async () => {
        ...
        // })
        let statusRes = await $http.get(url + "/appointment-status-except-deleted", { headers: headers })
        Promise.all(statusRes).then(response => {
            let completedStatusId = response.data
            console.log("completedStatusId", completedStatusId);
        })
