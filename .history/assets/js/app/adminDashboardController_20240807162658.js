app.controller('AdminDashboardController', function ($scope, $http, $rootScope, $location, API, adminBreadcrumbService, $timeout, processSelect2Service) {
  ...
  $scope.fetchRevenueByDateOrMonth = (date, month) => {
    $scope.getRevenue(date, month).then(response => {
      if (date) {
      ...
      $scope.fetchRevenueByPreviousDateOrPreviousMonth(date, month)
    }).catch(error => {
  ...
  $scope.fetchRevenueByPreviousDateOrPreviousMonth = (date, month) => {
    ...
    $scope.getRevenue(previousDate, previousMonth).then(response => {
      if (date) {
  ...
  //   }
  //   if(month){
  //     if ($scope.venueByMonth !== undefined && $scope.revenuePreviousMonth !== undefined) {
  //       let monthRevenue = $scope.venueByMonth;
  //       let previousRevenue = $scope.revenuePreviousMonth;
  //       let growthPercentage = ((monthRevenue - previousRevenue) / (previousRevenue == 0 ? 1 : previousRevenue)) * 100;
  //       $scope.isGrowing = growthPercentage > 0
  //       if (growthPercentage != 0 && previousRevenue != 0) {
  //         $scope.growthPercentage = growthPercentage.toFixed(2);
  //       } else {
  //         $scope.growthPercentage = growthPercentage
  //       }
  ...
  $scope.getRevenueGrowthComparison = (date, month) => {
    ...
    const updateGrowthData = (currentRevenue, previousRevenue) => {
    ...
    if (date && $scope.venueToday !== undefined && $scope.venuePreviousToday !== undefined) {
      updateGrowthData($scope.venueToday, $scope.venuePreviousToday);
    }

    if (month && $scope.venueByMonth !== undefined && $scope.revenuePreviousMonth !== undefined) {
      updateGrowthData($scope.venueByMonth, $scope.revenuePreviousMonth);
    }
  };
  ...
  $scope.generateMMYYYYData = () => {
    $http.get(url + '/get-date-data').then(response => {
      ...
      const selectedItem = $scope.dateDataDB.find(item => item.value === moment(new Date()).format('MM-YYYY'));
      if (selectedItem) {
        $scope.formFilter.id = selectedItem.id;
      } else {
        console.log("No matching item found for current month-year");
      }
    })
  }
  ...
  $scope.initUI = () => {
    ...
    $('#filterByMonth').on('change', function () {
      $timeout(function () {
        ...
        console.log("$scope.filterByMonth",$scope.filterByMonth);
        let month=parseInt($scope.filterByMonth.split('-')[0])
        console.log("month vvv",month);
        
        // $scope.fetchRevenueByDateOrMonth(null, month);
      });
    });
  }
