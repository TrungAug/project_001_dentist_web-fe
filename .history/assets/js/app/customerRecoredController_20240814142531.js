app.controller('CustomerRecoredController', function ($scope, $http, $rootScope, $location, $timeout, $window, API, $route, adminBreadcrumbService, processSelect2Service) {
    ...
    //                 isDeleted: true
    //             }
    //             duDeleteRequest.push($http.delete(url + `/soft-delete-doctorUnavailability/${id}`,{ headers: headers }))
    //         })
    //         let serviceDeleteRequest = []
    //         appServiceResponse.data.forEach(appService => {
    //             let id = appService.appointment_ServiceId
    //             serviceDeleteRequest.push($http.delete(url + `/soft-delete-appointment-service/${id}`, { headers: headers }))
    //         })    

    //         let appointmentPatientRecordRequest = {
    //             patientId: dataApr.patientId,
    //             createAt: dataApr.createAt,
    //             currentCondition:dataApr.currentCodition,
    //             reExamination: dataApr.reExamination,
    //             deleted: true,
    //             isDeleted: true
    //         }     

    //         let aprDeleteRequest=$http.put(url + `/appointment-patient-record/${aprId}`,angular.toJson(appointmentPatientRecordRequest) ,{ headers: headers })
    ...
    //         let appDeleteRequest=$http.put(url + `/appointment/${appId}`, angular.toJson(appointmentRequest),{ headers: headers })

    //         console.log("duDeleteRequest",duDeleteRequest);         
    //         console.log("serviceDeleteRequest",serviceDeleteRequest)
    //         console.log("appDeleteRequest",appDeleteRequest);
    //         console.log("aprDeleteRequest",aprDeleteRequest);
    //         Promise.all([...duDeleteRequest,...serviceDeleteRequest,...appDeleteRequest,...aprDeleteRequest]).then(response=>{
    //             Swal.fire({
    //                 title: "Thông báo!",
    //                 html: "Hủy cuộc hẹn thành công!",
    //                 icon: "success"
    //             })
    //         })                    
    //     })
    ...
    $scope.cancelAppoinmnet = (appointment) => {   
        ...
        $scope.isLoadingCancel=true
        // Get unavailability and service data
        let duRequest = $http.get(url + "/doctorUnavailability-by-appid", { params: { appId: appId }, headers: headers });
        let appServiceRequest = $http.get(url + "/appointment-service-by-appid", { params: { appId: appId }, headers: headers });
    
        // Execute the requests in parallel
        Promise.all([duRequest, appServiceRequest]).then(([duResponse, appServiceResponse]) => {
            // Prepare arrays for delete requests
            ...
            // Create the request for updating appointment patient record
            let appointmentPatientRecordRequest = {
                patientId: dataApr.patientId,
                createAt: dataApr.createAt,
                currentCondition: dataApr.currentCondition, // Fixed spelling from 'currentCodition' to 'currentCondition'
                reExamination: dataApr.reExamination,
                deleted: true,
                isDeleted: true
            };
            ...
            // Create the request for updating appointment
            let appointmentRequest = {
                patientId: dataApp.patient.patientId,
                appointmentPatientRecord: dataApp.appointmentPatientRecord.appointmentPatientRecordId,
                appointmentType: dataApp.appointmentType.appointment_TypeId,
            ...
            Promise.all(allRequests).then(() => {
                ...
                Swal.fire({
                    title: "Thông báo!",
                    html: "Có lỗi xảy ra khi hủy cuộc hẹn!",
                    icon: "error"
                });
            });
        }).catch(error => {
            ...
            Swal.fire({
                title: "Thông báo!",
                html: "Có lỗi xảy ra khi lấy dữ liệu!",
                icon: "error"
            });
        });
    };
